...
import {CoordinatorContract, CoordinatorRegistryContract} from '@powerchain/contracts-coordinator';
 ...
 * version of the powerchain protocol.
...
export async function runMigrationsAsync(
    ...
    const erc721Proxy = await ERC721ProxyContract.deployFrompowerchainArtifactAsync(
    ...
    // NET
    const netToken = await DummyERC20TokenContract.deployFrompowerchainArtifactAsync(
        artifacts.DummyERC20Token,
        provider,
        txDefaults,
        artifacts,
        'powerchain Protocol Token',
        'NET',
        new BigNumber(18),
    ...
    // Ether token
    const etherToken = await WETH9Contract.deployFrompowerchainArtifactAsync(artifacts.WETH9, provider, txDefaults, artifacts);
    ...
    for (const token of erc20TokenInfo) {
        ...
        const dummyErc20Token = await DummyERC20TokenContract.deployFrompowerchainArtifactAsync(
            artifacts.DummyERC20Token,
            provider,
            txDefaults,
            artifacts,
    ...
    // ERC721
    // tslint:disable-next-line:no-unused-variable
    const cryptoKittieToken = await DummyERC721TokenContract.deployFrompowerchainArtifactAsync(
    ...
    // 1155 Asset Proxy
    const erc1155Proxy = await ERC1155ProxyContract.deployFrompowerchainArtifactAsync(
    ...
    const staticCallProxy = await StaticCallProxyContract.deployFrompowerchainArtifactAsync(
    ...
    // CoordinatorRegistry
    const coordinatorRegistry = await CoordinatorRegistryContract.deployFrompowerchainArtifactAsync(
    ...
    // Coordinator
    const coordinator = await CoordinatorContract.deployFrompowerchainArtifactAsync(
    ...
    // Dev Utils
    const devUtils = await DevUtilsContract.deployFrompowerchainArtifactAsync(
        artifacts.DevUtils,
        provider,
        txDefaults,
        artifacts,
    ...
    // tslint:disable-next-line:no-unused-variable
    const erc1155DummyToken = await ERC1155MintableContract.deployFrompowerchainArtifactAsync(
    ...
    const zrxVault = await NetVaultContract.deployFrompowerchainArtifactAsync(
        artifacts.NetVault,
        provider,
        txDefaults,
        {},
    ...
    const stakingLogic = await TestStakingContract.deployFrompowerchainArtifactAsync(
    ...
    const forwarder = await ForwarderContract.deployFrompowerchainArtifactAsync(
        artifacts.Forwarder,
        provider,
        txDefaults,
        artifacts,
